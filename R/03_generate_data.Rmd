...
data_abs %>% 
  ...
  annotate(geom = "rect", xmin = -Inf, xmax = Inf, ymin = 0, ymax = Inf, fill = colors[1], alpha = .16) + 
  annotate(geom = "rect", xmin = -Inf, xmax = Inf, ymin = -Inf, ymax = 0, fill = colors[2], alpha = .16) + 
...
data_abs_scen %>%
  ...
  geom_hline(yintercept = 0, size = .6, linetype = "31", color = "grey75") +
  geom_boxplot(color = "grey45", size = .6, width = .5, position = position_nudge(x = .05), 
               outlier.size = .9, outlier.shape = 1) +
  geom_point(color = "grey90", shape = "-", size = 11.3, position = position_nudge(x = -.31)) +
  geom_point(color = "grey45", shape = "-", size = 11.3, alpha = .05, position = position_nudge(x = -.31)) +
  stat_summary(geom = "point", shape = 18, size = 4.5, position = position_nudge(x = .05)) +
  facet_wrap(~distribution, nrow = 2, dir = "v") + 
  scale_y_continuous(expand = c(.03, .03), breaks = seq(-800, 1600, by = 400)) +
  scale_color_identity() +
...
ggsave(here("plots", "bat_passes", "passes_difference_boxplots_abs_scenarios.pdf"), width = 12, height = 8.2, device = cairo_pdf)
...
ggsave(here("plots", "bat_passes", "passes_difference_boxplots_rel_scenarios.pdf"), width = 12, height = 8.2, device = cairo_pdf)
```
