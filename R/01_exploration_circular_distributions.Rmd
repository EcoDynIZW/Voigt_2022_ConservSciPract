...
```{r trials, eval=FALSE, include=FALSE}
### trials
pp <- spatstat.core::rpoint(n, f = function(x,y) {100 * exp(-5*y)}, win = circle)
df <- as.data.frame(pp)
base + geom_point(data = slice(df, 1:n), alpha = .7) + ggtitle("**Random")
```
